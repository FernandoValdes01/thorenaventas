...
export const orderBodySchema = z.object({
  ...
  customDiscountRate: z.number().min(0).max(0.95).default(0),
  customDiscountAmount: z.number().min(0).default(0),
  ...
  items: z.array(z.object({
    ...
    customDiscountRate: z.number().min(0).max(0.95).default(0),
    customDiscountAmount: z.number().min(0).default(0),
    itemKind: z.enum(['product', 'promotion']).default('product'),
    promoComponents: z.array(z.object({ productId: z.string().min(1), quantity: z.number().int().min(1) })).default([]),
